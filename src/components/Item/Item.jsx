...
export const Item = ({ language, data, i }) => {
    ...
    const orderProduct = (name, id) => {
        ...
        successMessage(language === 'RU' ? `Заказ - "${name}" добавлен в Ваши заказы` : language === 'TR' ? `Sipariş - ${name} siparişlerinize eklendi` : language === 'EN' ? `Order - ${name} added to your cart` : '')
    }
    ...
    return (
        <div className={Styles.item} data-aos="fade-left" data-aos-delay={i * 50}>
            ...
            <div className={Styles.desc}>
                ...
                <p>{ data.price } {language === 'RU' ? 'сом' : language === 'TR' ? 'som' : language === 'EN' ? 'som' : ''}</p>
                <div className={Styles.add}>
                    ...
                    <button onClick={() => {orderProduct(data.name, data.id)}}>{language === 'RU' ? 'Заказать' : language === 'TR' ? 'Sipariş ver' : language === 'EN' ? 'Order' : ''}</button>
                </div>
            </div>
            <div className={`${Styles.full} ${full === data.id ? Styles.active : ''}`}>
                ...
                <div className={Styles.text}>
                    ...
                    <p>{ data.weight } {language === 'RU' ? 'г.' : language === 'TR' ? 'g.' : language === 'EN' ? 'g.' : ''}</p>
                </div>
            </div>
        </div>
    )
